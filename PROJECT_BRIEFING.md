...
**As of 2026-08-06 (end of day):** read `DAILY_LOG.md` (newest entry first) for the actual day-by-day record — it is far more current and detailed than this section. In brief, the project now runs on:
- `COMMAND_CENTER.md` (this branch) — standing trigger phrases (`#newtask`, `#projectdevelopment`, `#totalcheck`, `#GoodMorning`, `#ShowMeMyKanban`, `#hardrule`, `#execute`, `#refreshmenow`, `#STOP`). **Read this before doing anything** — it governs how work actually gets done here.
...
- `Umbrella_Tasks.md` (`006_task-management-pm`) — 14 categories now (Logistics added 6 Aug), the shared vocabulary tagging every task/log entry.
...
- **The COC rebuild is visibly happening**, just not in git yet: Shagun's own `260804_K5M_Internal Central COC.xlsx` (Desktop) has 5 sheets with real content — canonical scope, an evolved Material Package (Fabrics/Hard Material), and the Shop Drawing Tracker populated with all 17 FFE items' SHD00 feedback (63 remarks, all unresolved). Worth committing to git once Shagun's ready; don't rebuild it independently in `003_trackers` in the meantime.
- **Material Package restructure, mid-discussion, not resolved:** three trackers — Main Central (= the COC), Client-facing, Supplier-facing — same column format across all three, diverging only past a sensitivity threshold. Where exactly that divergence point is still hasn't been pinned down.
- **Mail-monitoring system designed but blocked**: `#GoodMorning`/`#checkmail` would check Outlook for "K5M"-tagged mail and log it to `004_communication/K5M_Mail_Log.md`, cross-checking against the Task Tracker/Dev Log for anything unrecorded. Blocked at the Wood Couture M365 tenant level (an Application Access Policy denies the connector mailbox access) — needs IT/admin action, not something fixable from a session.
- Still carried forward from 4 Aug: a merged `Project Management_Tasks and Calendar.xlsx` workbook is scoped and agreed but not yet built; the Fitout scope base file is still undecided; `#projectdevelopment` logging gaps should be checked with Shagun directly each `#totalcheck` rather than assumed closed.
