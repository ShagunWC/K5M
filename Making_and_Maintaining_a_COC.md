...
9. **The COC is the master — client-facing and supplier-facing trackers are downstream copies.** Early on, it's tempting to build the COC by pulling content in from whatever audience-facing tracker already exists (supplier's version, client's version), since those get built first and get real-world feedback sooner. Reverse this as soon as the COC exists: the COC becomes the one place scope/material truth lives and gets edited, and every audience-facing copy gets updated *from* it — not the other way around. Decide case by case what actually flows into each downstream copy; don't assume it's a full mirror.
